...
function CartItemAmount ({product, cartArr, setCartArr}) {
    return (
        <div className="cartItemAmount">
            <p className='' onClick={()=> handleChangeAmount.subtractAmount(product, cartArr, setCartArr)} >-</p>
            <p className='productAmount'>{product.amount}</p>
            <p className='' onClick={()=>{handleChangeAmount.addAmount(product, cartArr, setCartArr)}} >+</p>
        </div>
    )
}
